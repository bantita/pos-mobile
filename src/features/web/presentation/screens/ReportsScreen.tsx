...
import {
  ...
  MOCK_SALES_BY_CASHIER,
  MOCK_SALES_BY_CATEGORY,
  MOCK_SALES_BY_DAY, MOCK_SALES_BY_MONTH,
  MOCK_SALES_SUMMARY,
  MOCK_STOCK_ITEMS,
  MOCK_TOP_PRODUCTS,
...
const BarChart: React.FC<{ data: { label: string; value: number; value2?: number }[]; color?: string; color2?: string; height?: number }> = ({
  data, color = Palette.primary, color2 = Palette.success, height = 120,
}) => {
    ...
    <View style={{ height, flexDirection: 'row', alignItems: 'flex-end', gap: 4 }}>
      {data.map((d, i) => (
        <View key={i} style={{ flex: 1, alignItems: 'center', height: '100%', justifyContent: 'flex-end' }}>
          ...
          <Text style={{ fontSize: 12, color: Palette.textSecondary, marginTop: 3 }}>{d.label}</Text>
        </View>
      ))}
    </View>
  );
...
const dr: Record<string, any> = {
  btn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: Palette.border, backgroundColor: '#fafafa' },
  btnActive: { backgroundColor: Palette.primary, borderColor: Palette.primary },
  btnText: { fontSize: 13, color: Palette.textSecondary },
  btnTextActive: { color: '#fafafa', fontWeight: '700' },
};
    ...
    <View style={{ flex: 1 }}>
      ...
      {trend !== undefined && (
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 2 }}>
          <Ionicons name={trend >= 0 ? 'trending-up-outline' : 'trending-down-outline'} size={11} color={trend >= 0 ? Palette.success : Palette.danger} />
          <Text style={{ fontSize: 12, color: trend >= 0 ? Palette.success : Palette.danger, fontWeight: '600' }}>
            {trend >= 0 ? '+' : ''}{trend}%
          </Text>
        </View>
      )}
...
const kpi: Record<string, any> = {
  card: { flex: 1, backgroundColor: '#fafafa', borderRadius: 12, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12, borderLeftWidth: 4, borderWidth: 1, borderColor: Palette.border },
  icon: { width: 44, height: 44, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  label: { fontSize: 13, color: Palette.textSecondary },
  value: { fontSize: 16, fontWeight: '800', marginTop: 2 },
  sub: { fontSize: 13, color: Palette.textSecondary },
};

// ─── Table Shell ──────────────────────────────────────────────────────────────
const TableShell: React.FC<{ cols: [string, number][]; rows: React.ReactNode[] }> = ({ cols, rows }) => {
  const { width: tableVW } = useWindowDimensions();
  const needsScroll = tableVW < 640;
  const content = (
    <View style={[tbl.wrap, needsScroll && { minWidth: 520 }]}>
      <View style={tbl.head}>
...
const tbl: Record<string, any> = {
  wrap: { backgroundColor: '#fafafa', borderRadius: 12, borderWidth: 1, borderColor: Palette.border },
  head: { flexDirection: 'row', backgroundColor: Palette.gray50, paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: Palette.border },
  th: { fontSize: 13, fontWeight: '700', color: Palette.textSecondary, textTransform: 'uppercase' },
  tr: { flexDirection: 'row', paddingHorizontal: 14, paddingVertical: 10, borderBottomWidth: 1, borderBottomColor: Palette.border, alignItems: 'center' },
  trAlt: { backgroundColor: Palette.gray50 },
  td: { fontSize: 12, color: Palette.text },
};
...
const SalesReport: React.FC = () => {
  ...
  const { width: rptWidth } = useWindowDimensions();
  const isNarrow = rptWidth < 768;
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: isNarrow ? 'column' : 'row', justifyContent: 'space-between', alignItems: isNarrow ? 'flex-start' : 'center', gap: isNarrow ? 8 : 0 }}>
        <DateRangeBar selected={dateRange} onChange={setDateRange} />
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <TouchableOpacity style={rpt.exportBtn}><Ionicons name="document-text-outline" size={14} color={Palette.success} /><Text style={[rpt.exportText, { color: Palette.success }]}>Excel</Text></TouchableOpacity>
          <TouchableOpacity style={[rpt.exportBtn, { borderColor: Palette.danger }]}><Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText, { color: Palette.danger }]}>PDF</Text></TouchableOpacity>
        </View>
      </View>

      <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
        <KpiCard label="ยอดขายรวม" value={`฿${fmt(s.totalSales)}`} icon="cash-outline" color={Palette.primary} trend={12} />
        <KpiCard label="จำนวนบิล" value={`${s.totalBills}`} sub={`ยกเลิก ${s.cancelledBills} บิล`} icon="receipt-outline" color={Palette.success} trend={5} />
        {!isNarrow && <KpiCard label="เฉลี่ย/บิล" value={`฿${fmt(s.avgPerBill)}`} icon="analytics-outline" color="#7c3aed" />}
        {!isNarrow && <KpiCard label="ส่วนลดรวม" value={`฿${fmt(s.totalDiscount)}`} icon="pricetag-outline" color="#f59e0b" />}
      </View>
      ...
      <View style={{ flexDirection: isNarrow ? 'column' : 'row', gap: 12 }}>
        ...
        <View style={[rpt.card, { flex: 1.5 }]}>
          ...
          <View style={{ flexDirection: 'row', gap: 16, marginTop: 8, justifyContent: 'center' }}>
            {[{color: Palette.primary, label:'ยอดขาย'},{color:Palette.success+'50',label:'กำไร'}].map(l => (
              <View key={l.label} style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                <View style={{ width: 10, height: 10, borderRadius: 8, backgroundColor: l.color }} />
                <Text style={{ fontSize: 13, color: Palette.textSecondary }}>{l.label}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[rpt.card, { flex: 1 }]}>
          ...
          {MOCK_SALES_BY_CATEGORY.map((c, i) => (
            <View key={i} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <View style={[rpt.rankBadge, i < 3 && { backgroundColor: Palette.primary }]}>
                <Text style={[{ fontSize: 12, fontWeight: '700', color: i < 3 ? '#fafafa' : Palette.textSecondary }]}>{i + 1}</Text>
              </View>
              <Text style={{ flex: 1, fontSize: 12, color: Palette.text }}>{c.categoryName}</Text>
              <View style={{ width: 80, height: 6, backgroundColor: Palette.gray100, borderRadius: 3, overflow: 'hidden' }}>
                <View style={{ width: `${c.percent}%` as any, height: '100%', backgroundColor: Palette.primary, borderRadius: 3 }} />
              </View>
              <Text style={{ fontSize: 13, fontWeight: '700', color: Palette.primary, width: 60, textAlign: 'right' }}>฿{c.sales.toLocaleString()}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      <View style={rpt.card}>
        ...
        <View style={{ flexDirection: 'row', gap: 24, flexWrap: 'wrap' }}>
          {[
            { label: 'เงินสด', amt: s.cashAmount, icon: 'cash-outline', color: Palette.success },
            { label: 'QR Code', amt: s.qrAmount, icon: 'qr-code-outline', color: Palette.primary },
            ...
            <View key={i} style={{ alignItems: 'center', gap: 4, flex: 1 }}>
              ...
              <Text style={{ fontSize: 13, color: Palette.textSecondary }}>{p.label}</Text>
            </View>
          ))}
        </View>
      </View>
        ...
        <TableShell
          ...
          rows={MOCK_SALES_BY_CASHIER.map((c, i) => (
            <View key={i} style={[tbl.tr, i % 2 === 1 && tbl.trAlt]}>
              ...
              <Text style={[tbl.td, { flex: 1, color: Palette.primary, fontWeight: '700' }]}>฿{fmt(c.sales)}</Text>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
        <TouchableOpacity style={rpt.exportBtn} onPress={() => exportGenericExcel('รายงานสรุปการขาย', pdfHeaders, excelRows, 'SalesSummaryReport')}>
          <Ionicons name="document-text-outline" size={14} color={Palette.success} />
          <Text style={[rpt.exportText, { color: Palette.success }]}>Excel</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[rpt.exportBtn, { borderColor: Palette.danger }]} onPress={() => printGenericPDF('รายงานสรุปการขาย', pdfHeaders, excelRows)}>
          <Ionicons name="document-outline" size={14} color={Palette.danger} />
          <Text style={[rpt.exportText, { color: Palette.danger }]}>PDF</Text>
        </TouchableOpacity>
      </View>
      <View style={{ flexDirection: 'row', gap: 10 }}>
        <KpiCard label="ยอดขายรวม"   value={`฿${fmt(s.totalSales)}`}    icon="cash-outline"      color={Palette.primary} trend={12} />
        <KpiCard label="จำนวนบิล"    value={`${s.totalBills} บิล`}       icon="receipt-outline"   color={Palette.success} />
        <KpiCard label="ยกเลิก"       value={`${s.cancelledBills} บิล`}  icon="close-circle-outline" color={Palette.danger} />
        ...
        <TableShell
          ...
          rows={MOCK_SALES_BY_DAY.map((d,i) => (
            <View key={i} style={[tbl.tr, i%2===1&&tbl.trAlt]}>
              ...
              <Text style={[tbl.td,{flex:1,color:Palette.primary,fontWeight:'700'}]}>฿{fmt(d.sales)}</Text>
              <Text style={[tbl.td,{flex:1,color:Palette.success}]}>฿{fmt(d.profit)}</Text>
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
        <TouchableOpacity style={rpt.exportBtn} onPress={() => exportGenericExcel('ภาษีขายแยกตามร้านค้า', ['เลขที่บิล','วันที่','แคชเชียร์','ก่อน VAT','VAT 7%','รวม VAT','ประเภท'], allRows, 'TaxByShopReport')}>
          <Ionicons name="document-text-outline" size={14} color={Palette.success} /><Text style={[rpt.exportText,{color:Palette.success}]}>Excel</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('ภาษีขายแยกตามร้านค้า', ['เลขที่บิล','วันที่','แคชเชียร์','ก่อน VAT','VAT 7%','รวม VAT','ประเภท'], allRows)}>
          <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
        </TouchableOpacity>
      </View>
      {shops.map((shop, si) => {
        ...
        return (
          <View key={si} style={rpt.card}>
            ...
            <TableShell
              ...
              rows={rows.map((inv,i) => (
                <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
                  <Text style={[tbl.td,{flex:1.4,color:Palette.primary,fontWeight:'600'}]}>{inv.billNo}</Text>
                  ...
                  <Text style={[tbl.td,{flex:1,textAlign:'right',fontWeight:'700',color:Palette.primary}]}>฿{fmt(inv.grand)}</Text>
                  <View style={{flex:0.9,justifyContent:'center'}}>
                    <View style={[rpt.marginBadge,{backgroundColor:inv.taxType==='tax_full'?'#ede9fe':Palette.primaryLight}]}>
                      <Text style={{fontSize: 12,fontWeight:'700',color:inv.taxType==='tax_full'?'#7c3aed':Palette.primary}}>{inv.taxType==='tax_full'?'เต็ม':'ย่อ'}</Text>
                    </View>
                  </View>
                </View>
              ))}
            />
            <View style={{flexDirection:'row',justifyContent:'flex-end',gap:16,paddingTop:8,borderTopWidth:1,borderTopColor:Palette.border}}>
              <Text style={{fontSize: 13,color:Palette.textSecondary}}>รวม {rows.length} รายการ</Text>
              <Text style={{fontSize: 12,fontWeight:'800',color:'#7c3aed'}}>VAT ฿{fmt(totV)}</Text>
              <Text style={{fontSize: 12,fontWeight:'800',color:Palette.primary}}>รวม ฿{fmt(totG)}</Text>
            </View>
          </View>
        );
      })}
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
        ...
        <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('สรุปยอดรับจ่ายแยกร้านค้า', ['ร้านค้า','เงินสด','QR','โอน','รวม'], excelRows)}>
          <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
        </TouchableOpacity>
      </View>
      <View style={rpt.card}>
        ...
        <TableShell
          ...
          rows={data.map((d,i) => (
            <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
              ...
              <Text style={[tbl.td,{flex:1,color:Palette.success}]}>฿{d.cash.toLocaleString()}</Text>
              <Text style={[tbl.td,{flex:1,color:Palette.primary}]}>฿{d.qr.toLocaleString()}</Text>
              <Text style={[tbl.td,{flex:1,color:'#7c3aed'}]}>฿{d.transfer.toLocaleString()}</Text>
              <Text style={[tbl.td,{flex:1,fontWeight:'800',color:Palette.text}]}>฿{d.total.toLocaleString()}</Text>
            </View>
          ))}
        />
      </View>
...
const SalesByPOSReport: React.FC = () => {
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
      ...
      <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('สรุปการขายแยกตามเครื่อง POS', ['รหัสเครื่อง','สาขา','ยอดขาย','จำนวนบิล','เฉลี่ย/บิล'], excelRows)}>
        <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
      </TouchableOpacity>
    </View>
    <View style={rpt.card}>
      ...
      <TableShell
        ...
        rows={MOCK_POS_PERFORMANCE.map((p,i) => (
          <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
            ...
            <Text style={[tbl.td,{flex:1,color:Palette.primary,fontWeight:'700'}]}>฿{p.sales.toLocaleString()}</Text>
...
const SalesByCashierReport: React.FC = () => {
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
      ...
      <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('สรุปการขายแยกตามผู้ขาย', ['พนักงาน','POS','ยอดขาย','จำนวนบิล','เฉลี่ย/บิล'], excelRows)}>
        <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
      </TouchableOpacity>
    </View>
    <View style={rpt.card}>
...
const SalesByProductReport: React.FC = () => {
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
    <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
      ...
      <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('สรุปการขายแยกตามสินค้า', ['รหัส','ชื่อสินค้า','หมวด','จำนวนขาย','รายได้','Margin'], excelRows)}>
        <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
      </TouchableOpacity>
    </View>
    <View style={rpt.card}>
      ...
      <TableShell
        ...
        rows={MOCK_TOP_PRODUCTS.map((p,i) => (
          <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
            ...
            <Text style={[tbl.td,{flex:1,color:Palette.primary,fontWeight:'700'}]}>฿{fmt(p.revenue)}</Text>
            <View style={{flex:0.7,justifyContent:'center'}}>
              <View style={[rpt.marginBadge,{backgroundColor:p.margin>=30?'#d1fae5':'#fef3c7'}]}>
                <Text style={{fontSize: 13,fontWeight:'700',color:p.margin>=30?Palette.success:'#f59e0b'}}>{p.margin.toFixed(1)}%</Text>
              </View>
            </View>
          </View>
        ))}
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <View style={{ flexDirection: 'row', gap: 10, flex: 1 }}>
          ...
          <KpiCard label="ยอดรวมโอน" value={`฿${fmt(transfers.reduce((s,r)=>s+r.grand,0))}`} icon="cash-outline" color={Palette.primary} />
        </View>
        <View style={{ flexDirection: 'row', gap: 8, paddingTop: 4, flexShrink: 0 }}>
          ...
          <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายการโอนเงิน (Transfer)', ['เลขที่บิล','วันที่','แคชเชียร์','ยอด'], excelRows)}>
            <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
      <View style={rpt.card}>
        ...
        <TableShell
          ...
          rows={transfers.map((r,i) => (
            <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
              <Text style={[tbl.td,{flex:1.4,color:Palette.primary,fontWeight:'600'}]}>{r.billNo}</Text>
              <Text style={[tbl.td,{flex:0.8,fontSize: 13}]}>{r.date}</Text>
              <Text style={[tbl.td,{flex:1.2}]}>{r.cashier}</Text>
              <Text style={[tbl.td,{flex:1,fontWeight:'700',color:Palette.primary,textAlign:'right'}]}>฿{fmt(r.grand)}</Text>
            </View>
          ))}
        />
      </View>
...
const BillPaymentReport: React.FC = () => {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <View style={{ flexDirection: 'row', gap: 10, flex: 1 }}>
          <KpiCard label="จำนวนรายการ Bill" value={`${bills.length} รายการ`} icon="qr-code-outline" color={Palette.primary} />
          <KpiCard label="ยอดรวม" value={`฿${fmt(bills.reduce((s,r)=>s+r.grand,0))}`} icon="cash-outline" color={Palette.success} />
        </View>
        <View style={{ flexDirection: 'row', gap: 8, paddingTop: 4, flexShrink: 0 }}>
          ...
          <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายการชำระบิล (Bill Payment)', ['เลขที่บิล','วันที่','แคชเชียร์','ยอด'], excelRows)}>
            <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
...
const VoidSalesReport: React.FC = () => {
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <View style={{ flexDirection: 'row', gap: 10, flex: 1 }}>
          <KpiCard label="จำนวนยกเลิก" value={`${voids.length} รายการ`} icon="close-circle-outline" color={Palette.danger} />
        ...
        <View style={{ flexDirection: 'row', gap: 8, paddingTop: 4, flexShrink: 0 }}>
          ...
          <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายงานการยกเลิกการขาย', ['เลขที่บิล','วันที่','แคชเชียร์','เหตุผล','ยอด'], excelRows)}>
            <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
      <View style={rpt.card}>
        ...
        <TableShell
          ...
          rows={voids.map((r,i) => (
            <View key={i} style={[tbl.tr,i%2===1&&tbl.trAlt]}>
              <Text style={[tbl.td,{flex:1.4,color:Palette.danger,fontWeight:'600'}]}>{r.billNo}</Text>
              <Text style={[tbl.td,{flex:0.8,fontSize: 13}]}>{r.date}</Text>
              <Text style={[tbl.td,{flex:1.2}]}>{r.cashier}</Text>
              <Text style={[tbl.td,{flex:1.5,color:Palette.textSecondary}]}>{r.reason}</Text>
              <Text style={[tbl.td,{flex:0.9,fontWeight:'700',color:Palette.danger,textAlign:'right'}]}>฿{fmt(Math.abs(r.grand))}</Text>
            </View>
          ))}
        />
      </View>
...
function exportGenericExcel(title: string, headers: string[], rows: string[][], filename: string) {
  ...
  const ths = headers.map(h => `<th style="background:#f4f4f5;font-weight:bold;border:1px solid #d6d3d1;padding:6px 10px">${h}</th>`).join('');
  const trs = rows.map((r, i) =>
    `<tr>${r.map(c => `<td style="border:1px solid #d4d4d8;padding:5px 10px;${i % 2 === 1 ? 'background:#fafafa' : ''}">${c}</td>`).join('')}</tr>`
  ).join('');
...
function printGenericPDF(title: string, headers: string[], rows: string[][], subtitle?: string) {
  ...
  const trs = rows.map((r, i) =>
    `<tr style="${i%2===1?'background:#fafaf9':''}">${r.map(c=>`<td>${c}</td>`).join('')}</tr>`
  ).join('');
  const html = `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"/>
<title>${title}</title>
<style>body{font-family:'Sarabun',Arial,sans-serif;font-size:13px;margin:24px}
h2{font-size:16px;margin:0 0 4px}.sub{font-size:12px;color:#404040;margin:0 0 2px}
table{width:100%;border-collapse:collapse;margin-top:16px}
th{background:#e5e7eb;border:1px solid #a1a1aa;padding:7px 8px;font-size:12px;text-align:center}
td{border:1px solid #d6d3d1;padding:6px 8px;font-size:12px}
...
export const ReportsScreen: React.FC<Props> = ({ onNavigate, initialView }) => {
  ...
  const renderNavGroup = (
    ...
    <>
      <TouchableOpacity style={rpt.navGroupHeader} onPress={onToggle} activeOpacity={0.8}>
        ...
        <Ionicons name={expanded ? 'chevron-up' : 'chevron-down'} size={12} color={Palette.textSecondary} />
      </TouchableOpacity>
  ...
  return (
    <View style={[rpt.screenRoot, isCompact && rpt.screenRootCompact]}>

      {/* ── LEFT SIDEBAR ── */}
      <View style={[
        rpt.navPanel,
        isCompact && rpt.navPanelCompact,
        isCompact && sidebarOpen && rpt.navPanelCompactOpen,
        !sidebarOpen && (isCompact ? rpt.navPanelCompactClosed : { width: 36 }),
      ]}>
        {/* Toggle button */}
        <TouchableOpacity
          style={[rpt.navToggleBtn, isCompact && rpt.navToggleBtnCompact]}
          onPress={() => setSidebarOpen(o => !o)}
          activeOpacity={0.7}
        >
          {isCompact && (
            <View style={rpt.navToggleContent}>
              <View style={rpt.navToggleIcon}>
                <Ionicons name="bar-chart-outline" size={16} color={Palette.primary} />
              </View>
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text style={rpt.navToggleEyebrow}>เลือกรายงาน</Text>
                <Text style={rpt.navToggleLabel} numberOfLines={1}>{currentLabel}</Text>
              </View>
            </View>
          )}
          <Ionicons
            name={isCompact ? (sidebarOpen ? 'chevron-up' : 'chevron-down') : (sidebarOpen ? 'chevron-back' : 'chevron-forward')}
        ...
        {sidebarOpen && (
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }}>
            {renderNavGroup('bar-chart-outline', 'รายงานการขาย', Palette.primary, Palette.primaryLight,
      ...
      <View style={rpt.contentPanel}>
        ...
        <View style={rpt.filterBar}>
          <View style={[rpt.filterRow, isCompact && rpt.filterRowCompact]}>
            ...
            <View style={rpt.filterField}>
              ...
              <View style={rpt.filterInputWrap}>
                ...
                <TextInput
                  ...
                  placeholder="วว/ดด/ปปปป" placeholderTextColor={Palette.textDisabled}
            ...
            <TouchableOpacity style={[rpt.searchBtn, { alignSelf: 'flex-end' }]}>
              <Ionicons name="search-outline" size={13} color="#fafafa" />
        ...
        {/* ── KPI row ── */}
        <View style={[rpt.kpiRow, isCompact && rpt.kpiRowCompact]}>
          {[
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          ...
          <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายงานสินค้า', ['รหัส','ชื่อสินค้า','หมวด','จำนวนขาย','รายได้','กำไร','Margin'], excelRows)}>
            <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
      <View style={{ flexDirection: 'row', gap: 10 }}>
        <KpiCard label="รายได้รวม" value={`฿${fmt(totalRev)}`} icon="cash-outline" color={Palette.primary} />
        <KpiCard label="กำไรรวม" value={`฿${fmt(totalProf)}`} icon="trending-up-outline" color={Palette.success} />
          ...
          rows={MOCK_TOP_PRODUCTS.map((p, i) => (
            <View key={i} style={[tbl.tr, i % 2 === 1 && tbl.trAlt]}>
              <View style={[{ flex: 0.4, alignItems: 'center' }]}>
                <View style={[kpi.icon, { width: 22, height: 22, borderRadius: 11, backgroundColor: i < 3 ? Palette.primary : Palette.gray100 }]}>
                  <Text style={{ fontSize: 12, fontWeight: '700', color: i < 3 ? '#fafafa' : Palette.textSecondary }}>{i+1}</Text>
                </View>
              </View>
              <Text style={[tbl.td, { flex: 0.8 }]}>{p.productCode}</Text>
              ...
              <Text style={[tbl.td, { flex: 1, color: Palette.primary, fontWeight: '700' }]}>฿{fmt(p.revenue)}</Text>
              <Text style={[tbl.td, { flex: 0.9, color: Palette.success }]}>฿{fmt(p.profit)}</Text>
              <View style={{ flex: 0.7, justifyContent: 'center' }}>
                <View style={[rpt.marginBadge, { backgroundColor: p.margin >= 30 ? '#d1fae5' : '#fef3c7' }]}>
                  <Text style={{ fontSize: 13, fontWeight: '700', color: p.margin >= 30 ? Palette.success : '#f59e0b' }}>{p.margin.toFixed(1)}%</Text>
                </View>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
        ...
        <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายงานคลังสินค้า', ['รหัส','ชื่อสินค้า','คลัง','คงเหลือ','มูลค่า','สถานะ'], excelRows)}>
          <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
        </TouchableOpacity>
      </View>
      <View style={{ flexDirection: 'row', gap: 10 }}>
        {[
          { label: 'SKU ทั้งหมด',  value: String(MOCK_STOCK_ITEMS.length),                                color: Palette.primary },
          { label: 'ใกล้หมด',      value: String(MOCK_STOCK_ITEMS.filter(i=>i.status==='low').length),  color: '#f59e0b' },
          { label: 'หมดสต๊อก',    value: String(MOCK_STOCK_ITEMS.filter(i=>i.status==='out').length),  color: Palette.danger },
          { label: 'มูลค่ารวม',    value: `฿${fmt(totalVal)}`,                                           color: Palette.success },
      ...
      <View style={rpt.card}>
        ...
        <TableShell
          ...
          rows={MOCK_STOCK_ITEMS.map((item, i) => {
            const stColor = item.status==='ok' ? Palette.success : item.status==='low' ? '#f59e0b' : Palette.danger;
            ...
            return (
              <View key={i} style={[tbl.tr, i%2===1 && tbl.trAlt]}>
                ...
                <Text style={[tbl.td, { flex: 0.9, fontWeight: '700', color: item.onHandQty <= item.minStock ? Palette.danger : Palette.text }]}>{item.onHandQty} {item.unit}</Text>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <View style={{ flexDirection: 'row', gap: 10, flex: 1 }}>
          <KpiCard label="รายได้รวม" value={`฿${fmt(totalRev)}`} icon="cash-outline" color={Palette.primary} />
          <KpiCard label="กำไรขั้นต้น" value={`฿${fmt(totalProf)}`} icon="trending-up-outline" color={Palette.success} />
        ...
        <View style={{ flexDirection: 'row', gap: 8, paddingLeft: 12, flexShrink: 0 }}>
          ...
          <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('รายงานกำไร', getHeaders(), getExcelRows())}>
            <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {/* View mode toggle */}
      <View style={{ flexDirection: 'row', backgroundColor: Palette.gray100, borderRadius: 8, padding: 3, alignSelf: 'flex-start', gap: 3 }}>
        {([['day','รายวัน'],['month','รายเดือน'],['product','ตามสินค้า']] as const).map(([k, l]) => (
          <TouchableOpacity key={k} style={[{ paddingHorizontal: 14, paddingVertical: 7, borderRadius: 8 }, viewMode === k && { backgroundColor: '#fafafa' }]} onPress={() => setViewMode(k)}>
            <Text style={{ fontSize: 13, fontWeight: viewMode === k ? '700' : '500', color: viewMode === k ? Palette.text : Palette.textSecondary }}>{l}</Text>
          </TouchableOpacity>
        ))}
      </View>
          ...
          rows={viewMode === 'product'
            ? MOCK_PROFIT_BY_PRODUCT.map((p, i) => (
              <View key={i} style={[tbl.tr, i%2===1&&tbl.trAlt]}>
                ...
                <Text style={[tbl.td,{flex:1,color:Palette.danger}]}>฿{fmt(p.cost)}</Text>
                <Text style={[tbl.td,{flex:1,color:Palette.success,fontWeight:'700'}]}>฿{fmt(p.profit)}</Text>
                <View style={{flex:0.7,justifyContent:'center'}}>
                  <View style={[rpt.marginBadge,{backgroundColor:p.margin>=30?'#d1fae5':'#fef3c7'}]}>
                    <Text style={{fontSize: 13,fontWeight:'700',color:p.margin>=30?Palette.success:'#f59e0b'}}>{p.margin.toFixed(1)}%</Text>
                  </View>
            ...
            : (data ?? []).map((d, i) => (
              <View key={i} style={[tbl.tr, i%2===1&&tbl.trAlt]}>
                ...
                <Text style={[tbl.td,{flex:1.2,color:Palette.danger}]}>฿{fmt(d.cost)}</Text>
                <Text style={[tbl.td,{flex:1.2,color:Palette.success,fontWeight:'700'}]}>฿{fmt(d.grossProfit)}</Text>
                <View style={{flex:0.7,justifyContent:'center'}}>
                  <View style={[rpt.marginBadge,{backgroundColor:d.margin>=25?'#d1fae5':'#fef3c7'}]}>
                    <Text style={{fontSize: 13,fontWeight:'700',color:d.margin>=25?Palette.success:'#f59e0b'}}>{d.margin}%</Text>
                  </View>
                </View>
              </View>
            ))
...
function printTaxReportPDF(
  ...
  const rowsHtml = rows.map((r, i) => `
    <tr style="${i % 2 === 1 ? 'background:#fafaf9' : ''}">
...
<style>
  body { font-family: 'Sarabun', 'TH SarabunPSK', Arial, sans-serif; font-size: 13px; margin: 24px; color: #171717; position: relative; }
  h2   { font-size: 16px; margin: 0 0 2px; }
  .sub { font-size: 12px; color: #404040; margin: 0 0 2px; }
  .right-header { position: absolute; top: 0; right: 0; text-align: right; font-size: 11px; color: #525252; }
  table { width: 100%; border-collapse: collapse; margin-top: 18px; }
  th    { background: #e5e7eb; border: 1px solid #a1a1aa; padding: 7px 8px; font-size: 12px; text-align: center; vertical-align: middle; }
  td    { border: 1px solid #d6d3d1; padding: 6px 8px; font-size: 12px; vertical-align: middle; }
  tfoot td { background: #f4f4f5; font-weight: bold; border-top: 2px solid #525252; }
    ...
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 6 }}>
          ...
          <TouchableOpacity
            style={[rpt.exportBtn, { borderColor: Palette.danger }]}
      ...
      {/* KPI */}
      <View style={{ flexDirection: 'row', gap: 10, flexWrap: 'wrap' }}>
        <KpiCard label="ยอดขายก่อน VAT"    value={`฿${fmt(totalSubtotal)}`}  icon="receipt-outline"       color={Palette.primary}  />
        <KpiCard label="VAT 7% รวมทั้งสิ้น" value={`฿${fmt(totalVat)}`}       icon="calculator-outline"    color="#7c3aed"             />
        <KpiCard label="ยอดขายรวม VAT"      value={`฿${fmt(totalGrand)}`}     icon="cash-outline"          color={Palette.success}  />
      ...
      <View style={{ flexDirection: 'row', gap: 10 }}>
        {/* ใบกำกับภาษีอย่างย่อ */}
        <View style={[rpt.card, { flex: 1, borderTopWidth: 3, borderTopColor: Palette.primary }]}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            <Ionicons name="newspaper-outline" size={18} color={Palette.primary} />
            <Text style={[rpt.cardTitle, { color: Palette.primary }]}>ใบกำกับภาษีอย่างย่อ</Text>
          </View>
          <Text style={{ fontSize: 16, fontWeight: '900', color: Palette.primary }}>{totalShortTax} ใบ</Text>
          <View style={{ gap: 4 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
              <Text style={{ fontSize: 13, color: Palette.textSecondary }}>ยอดก่อน VAT</Text>
      ...
      <View style={rpt.card}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 6 }}>
            {([
              ...
              <TouchableOpacity
                key={f.key}
                style={[rpt.toggleBtn, typeFilter === f.key && { backgroundColor: Palette.primary, borderColor: Palette.primary }]}
                onPress={() => setTypeFilter(f.key)}
              >
                <Text style={[rpt.toggleText, typeFilter === f.key && { color: '#fafafa' }]}>{f.label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </View>
        ...
        <TableShell
          ...
          rows={filtered.map((inv, i) => (
            <View key={i} style={[tbl.tr, i % 2 === 1 && tbl.trAlt]}>
              <Text style={[tbl.td, { flex: 1.4, fontWeight: '600', color: Palette.primary }]}>{inv.billNo}</Text>
              ...
              <Text style={[tbl.td, { flex: 0.8, textAlign: 'right', color: '#7c3aed', fontWeight: '700' }]}>฿{fmt(inv.vat)}</Text>
              <Text style={[tbl.td, { flex: 0.9, textAlign: 'right', fontWeight: '700', color: Palette.primary }]}>฿{fmt(inv.grand)}</Text>
              <View style={{ flex: 1, justifyContent: 'center' }}>
                <View style={[rpt.marginBadge, {
                  backgroundColor: inv.taxType === 'tax_full' ? '#ede9fe' : Palette.primaryLight,
                }]}>
                  <Text style={{ fontSize: 12, fontWeight: '700', color: inv.taxType === 'tax_full' ? '#7c3aed' : Palette.primary }}>
        ...
        {/* Footer summary */}
        <View style={{ flexDirection: 'row', paddingHorizontal: 14, paddingVertical: 10, backgroundColor: Palette.primaryLight, borderRadius: 8, gap: 4 }}>
          <Text style={{ flex: 1.4 + 0.9 + 1.2 + 0.9, fontSize: 13, fontWeight: '700', color: Palette.primary }}>
            รวม {filtered.length} รายการ
          </Text>
          <Text style={{ flex: 0.9, textAlign: 'right', fontSize: 12, fontWeight: '800', color: Palette.text }}>฿{fmt(totalSubtotal)}</Text>
          <Text style={{ flex: 0.8, textAlign: 'right', fontSize: 12, fontWeight: '800', color: '#7c3aed' }}>฿{fmt(totalVat)}</Text>
          <Text style={{ flex: 0.9, textAlign: 'right', fontSize: 12, fontWeight: '900', color: Palette.primary }}>฿{fmt(totalGrand)}</Text>
          <View style={{ flex: 1 }} />
        </View>
      </View>
...
const EnterpriseReport: React.FC = () => {
  ...
  <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 16 }}>
    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
      <View style={{ flexDirection: 'row', gap: 10, flex: 1 }}>
        <KpiCard label="รายได้ทุกสาขา" value={`฿${fmt(MOCK_BRANCH_KPI.reduce((s,b)=>s+b.sales,0))}`} icon="business-outline" color={Palette.primary} />
        <KpiCard label="กำไรรวม" value={`฿${fmt(MOCK_BRANCH_KPI.reduce((s,b)=>s+b.profit,0))}`} icon="trending-up-outline" color={Palette.success} />
      ...
      <View style={{ flexDirection: 'row', gap: 8, paddingTop: 4, flexShrink: 0 }}>
        ...
        <TouchableOpacity style={[rpt.exportBtn,{borderColor:Palette.danger}]} onPress={() => printGenericPDF('Enterprise Report - KPI สาขา', ['สาขา','ยอดขาย','บิล','กำไร','Margin','Turnover','GMROI'], excelBranch)}>
          <Ionicons name="document-outline" size={14} color={Palette.danger} /><Text style={[rpt.exportText,{color:Palette.danger}]}>PDF</Text>
        </TouchableOpacity>
      </View>
    </View>
      ...
      <TableShell
        ...
        rows={MOCK_BRANCH_KPI.map((b, i) => (
          <View key={i} style={[tbl.tr, i%2===1&&tbl.trAlt]}>
            ...
            <Text style={[tbl.td,{flex:1.1,color:Palette.primary,fontWeight:'700'}]}>฿{b.sales.toLocaleString()}</Text>
            <Text style={[tbl.td,{flex:0.7}]}>{b.bills}</Text>
            <Text style={[tbl.td,{flex:1,color:Palette.success}]}>฿{b.profit.toLocaleString()}</Text>
            <View style={{flex:0.7,justifyContent:'center'}}><View style={[rpt.marginBadge,{backgroundColor:b.margin>=25?'#d1fae5':'#fef3c7'}]}><Text style={{fontSize: 13,fontWeight:'700',color:b.margin>=25?Palette.success:'#f59e0b'}}>{b.margin}%</Text></View></View>
            <View style={{flex:0.8,justifyContent:'center'}}><View style={[rpt.marginBadge,{backgroundColor:b.inventoryTurnover>=8?'#d1fae5':'#fef3c7'}]}><Text style={{fontSize: 13,fontWeight:'700',color:b.inventoryTurnover>=8?Palette.success:'#f59e0b'}}>{b.inventoryTurnover}x</Text></View></View>
            <View style={{flex:0.7,justifyContent:'center'}}><View style={[rpt.marginBadge,{backgroundColor:b.gmroi>=18?'#d1fae5':Palette.primaryLight}]}><Text style={{fontSize: 13,fontWeight:'700',color:b.gmroi>=18?Palette.success:Palette.primary}}>{b.gmroi}x</Text></View></View>
          </View>
        ))}
      />
    </View>
...
const ShiftReport: React.FC = () => {
  const shiftStore = require('@/features/sale/application/stores/shiftStore');
    ...
    <View style={{ gap: 14 }}>
      ...
      {allShifts.length === 0 ? (
        ...
        allShifts.map((sh: any, i: number) => (
          <View key={sh.id} style={{ backgroundColor: '#fafafa', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: Colors.border, gap: 8 }}>
            {/* Header */}
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
              <View>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flex: 1, backgroundColor: Colors.background, borderRadius: 8, padding: 8, alignItems: 'center' }}>
                <Text style={{ fontSize: 14, fontWeight: '800', color: Palette.primary }}>฿{sh.cashSalesTotal.toLocaleString()}</Text>
    ...
    <View style={{ gap: 15 }}>
      ...
      <View style={{ flexDirection: 'row', gap: 10 }}>
        <View style={{ flex: 1, backgroundColor: '#fafafa', borderRadius: 12, padding: 14, alignItems: 'center', borderWidth: 1, borderColor: Colors.border }}>
          <Text style={{ fontSize: 17, fontWeight: '800', color: Palette.primary }}>฿{totalAll.toLocaleString()}</Text>
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 8 }}>
        <TouchableOpacity style={rpt.exportBtn} onPress={() => {
          ...
          <Ionicons name="document-text-outline" size={13} color={Palette.success} />
          <Text style={[rpt.exportText, { color: Palette.success }]}>Excel</Text>
        </TouchableOpacity>
      </View>
      ...
      {mockStaffData.map((staff, idx) => (
        <View key={idx} style={{ backgroundColor: '#fafafa', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: Colors.border, gap: 10 }}>
          {/* Staff header */}
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <Text style={{ fontSize: 15, fontWeight: '800', color: Palette.primary }}>฿{staff.totalSales.toLocaleString()}</Text>
          </View>
          ...
          <View style={{ borderRadius: 8, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden' }}>
            ...
            {staff.services.map((sv, i) => (
              <View key={i} style={{ flexDirection: 'row', paddingVertical: 8, paddingHorizontal: 12, borderBottomWidth: i < staff.services.length - 1 ? 1 : 0, borderBottomColor: Colors.border, backgroundColor: i % 2 === 1 ? '#fafafa' : '#fafafa' }}>
                <Text style={{ flex: 1.2, fontSize: 11, color: Palette.primary, fontWeight: '500' }}>{sv.bill}</Text>
...
const DailySummaryReport: React.FC = () => {
  ...
  return (
    <View style={{ gap: 16 }}>
      ...
      {/* KPI */}
      <View style={{ flexDirection: 'row', gap: 10 }}>
        <View style={{ flex: 1, backgroundColor: '#fafafa', borderRadius: 12, padding: 16, alignItems: 'center', borderWidth: 1, borderColor: Colors.border }}>
          <Text style={{ fontSize: 16, fontWeight: '800', color: Palette.primary }}>฿{mockSummary.totalSales.toLocaleString()}</Text>
      ...
      {/* ช่องทางชำระ */}
      <View style={{ backgroundColor: '#fafafa', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: Colors.border, gap: 10 }}>
      ...
      {/* Top Products */}
      <View style={{ backgroundColor: '#fafafa', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: Colors.border, gap: 8 }}>
...
const rpt: Record<string, any> = {
  ...
  screenRootCompact: { flexDirection: 'column' },
  pageTitle: { fontSize: 16, fontWeight: '800', color: Palette.text },
  pageSub: { fontSize: 12, color: Palette.textSecondary, marginTop: -10 },
  cardGrid: { gap: 10 },
  reportCard: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: '#fafafa', borderRadius: 12, padding: 18, borderTopWidth: 4, borderWidth: 1, borderColor: Palette.border, position: 'relative' },
  reportIcon: { width: 52, height: 52, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  reportLabel: { fontSize: 12, fontWeight: '700', color: Palette.text },
  ...
  phase2Text: { fontSize: 12, color: '#fafafa', fontWeight: '800' },
  backBar: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: Palette.border },
  backBtn: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  backText: { fontSize: 12, color: Palette.primary },
  backTitle: { fontSize: 13, fontWeight: '700', color: Palette.text },
  card: { backgroundColor: '#fafafa', borderRadius: 12, padding: 16, gap: 12, borderWidth: 1, borderColor: Palette.border },
  cardTitle: { fontSize: 12, fontWeight: '700', color: Palette.text },
  exportBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, borderWidth: 1, borderColor: Palette.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 7 },
  exportText: { fontSize: 12, fontWeight: '600' },
  toggleBtn: { borderWidth: 1, borderColor: Palette.border, borderRadius: 8, paddingHorizontal: 12, paddingVertical: 5 },
  toggleText: { fontSize: 13, color: Palette.primary, fontWeight: '600' },
  rankBadge: { width: 22, height: 22, borderRadius: 11, backgroundColor: Palette.gray100, alignItems: 'center', justifyContent: 'center' },
  ...
  subMenuDot: {
    ...
    backgroundColor: Palette.primary, opacity: 0.6,
  },
  subMenuLabel: { flex: 1, fontSize: 12, color: Palette.text, fontWeight: '500' },
  ...
  navPanelCompact: { width: '100%' as any, height: 58, borderRightWidth: 0, borderBottomWidth: 1, borderBottomColor: Palette.border },
  navPanelCompactOpen: { height: 330 },
  navPanelCompactClosed: { width: '100%' as any, height: 58 },
  navToggleBtn: {
    height: 32, alignItems: 'center', justifyContent: 'center',
    borderBottomWidth: 1, borderBottomColor: Palette.border,
    backgroundColor: Palette.gray50,
  },
  navToggleBtnCompact: { height: 58, flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 14 },
  navToggleContent: { flex: 1, minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 10 },
  navToggleIcon: { width: 34, height: 34, borderRadius: 9, alignItems: 'center', justifyContent: 'center', backgroundColor: Palette.primaryLight },
  navToggleEyebrow: { fontSize: 10, fontWeight: '600', color: Palette.textSecondary },
  navToggleLabel: { marginTop: 1, fontSize: 13, fontWeight: '700', color: Palette.text },
  navGroupHeader: {
  ...
  navGroupLabel: { flex: 1, fontSize: 13, fontWeight: '700', color: Palette.text },
  navGroupSub:   { fontSize: 12, color: Palette.textSecondary },
  navItem: {
    paddingLeft: 14, paddingRight: 12, paddingVertical: 9,
    borderBottomWidth: 1, borderBottomColor: Palette.border,
  },
  navItemActive:      { backgroundColor: Palette.primaryLight },
  navDot:             { width: 5, height: 5, borderRadius: 3, backgroundColor: Palette.border, flexShrink: 0 },
  navItemLabel:       { fontSize: 13, color: Palette.textSecondary, lineHeight: 17 },
  navItemLabelActive: { color: Palette.primary, fontWeight: '700' },
  navDivider:         { height: 4, backgroundColor: Palette.gray100 },
  ...
  filterRowCompact: { flexWrap: 'wrap' },
  filterField: { flex: 1, minWidth: 140, gap: 4 },
  filterLabel: { fontSize: 13, fontWeight: '600', color: Palette.textSecondary },
  filterInputWrap: {
  ...
  filterInputText: { flex: 1, fontSize: 12, color: Palette.text },
  searchBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
  ...
  searchBtnText: { fontSize: 12, fontWeight: '700', color: '#fafafa' },
  ...
  kpiRowCompact: { gap: 8 },
  kpiCard: {
    flex: 1, minWidth: 140, backgroundColor: '#ffffff',
    borderRadius: 14, padding: 14,
    borderWidth: 1, borderColor: Palette.border,
    gap: 6,
  },
  kpiCardLabel: { fontSize: 12, color: Palette.textSecondary },
  kpiCardValue: { fontSize: 18, fontWeight: '900', color: Palette.text },
  ...
  contentTitle:     { fontSize: 12, fontWeight: '700', color: Palette.text },
  contentTitle2Wrap: { paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#fafafa', borderBottomWidth: 1, borderBottomColor: Palette.border, flexShrink: 0 },
  contentTitle2:     { fontSize: 12, fontWeight: '700', color: Palette.text },
};
