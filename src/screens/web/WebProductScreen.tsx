/**
 * WebProductScreen — M04 จัดการสินค้า
...
import {
  ...
  TextInput, Modal, ScrollView, Switch, Image, Platform, useWindowDimensions,
} from 'react-native';
import { Ionicons } from '@expo/vector-icons';
import { WebColors } from '../../constants/webColors';
import { Colors, Space, Radius, Shadow, Font } from '../../design-system/tokens';
import { MOCK_PRODUCTS, MOCK_CATEGORIES, MOCK_BRANDS, UNITS } from '../../data/mockProducts';
import { ProductMaster, ProductUOM } from '../../types/product';
    ...
    <View style={ur.card}>
      ...
      <View style={ur.cardHeader}>
        <View style={[ur.unitBadge, uom.isDefault && ur.unitBadgeDefault]}>
          <Text style={[ur.unitBadgeText, uom.isDefault && { color: WebColors.primary }]}>
      ...
      <View style={ur.fieldsRow}>
        ...
        <View style={[ur.col1, { zIndex: 99 }]}>
          ...
          >
            <Text style={[{ flex: 1, fontSize: 12 }, !uom.unit && { color: WebColors.textDisabled }]}>
              {uom.unit || 'เลือกหรือพิมพ์...'}
            </Text>
            <Ionicons name={showUnitDrop ? 'chevron-up' : 'chevron-down'} size={13} color={WebColors.textSecondary} />
          </TouchableOpacity>
              ...
              <ScrollView style={{ maxHeight: 180 }} nestedScrollEnabled showsVerticalScrollIndicator={false}>
                {filteredUnits.map(u => (
                  ...
                  >
                    ...
                    {uom.unit === u && <Ionicons name="checkmark" size={13} color={WebColors.primary} />}
                  </TouchableOpacity>
                ))}
                {filteredUnits.length === 0 && (
                  <Text style={{ padding: 12, fontSize: 13, color: WebColors.textDisabled }}>
        ...
        <View style={ur.col2}>
          ...
          <View style={ur.marginBox}>
            <Text style={[
              ur.marginText,
              { color: uom.salePrice > 0 && uom.salePrice > uom.costPrice ? WebColors.success : WebColors.danger },
            ]}>
              {uom.salePrice > 0
        ...
        <View style={ur.colDefault}>
          ...
          <Switch
            ...
            trackColor={{ true: WebColors.primary, false: WebColors.border }}
          />
        </View>
      </View>
      ...
      <View style={ur.barcodeSection}>
        <View style={ur.barcodeHeader}>
          <Ionicons name="barcode-outline" size={13} color={WebColors.textSecondary} />
        ...
        <View style={ur.bcContainer}>
          ...
          {uom.barcodes.map(bc => (
            <View key={bc} style={ur.bcChip}>
              <Ionicons name="barcode-outline" size={11} color={WebColors.primary} />
          ...
          <View style={ur.bcInputRow}>
            ...
            <TouchableOpacity style={ur.bcAddBtn} onPress={addBarcode} disabled={!bcInput.trim()}>
              <Ionicons name="add" size={15} color={bcInput.trim() ? WebColors.primary : WebColors.border} />
            </TouchableOpacity>
          </View>
        </View>
...
const ur = StyleSheet.create({
  ...
  unitBadge: {
    ...
    borderRadius: 20, backgroundColor: WebColors.gray100,
    borderWidth: 1, borderColor: WebColors.border,
  },
  unitBadgeDefault: { backgroundColor: WebColors.primaryLight, borderColor: WebColors.primary },
  unitBadgeText: { fontSize: 12, fontWeight: '700', color: WebColors.textSecondary },
  defaultTag: {
    backgroundColor: WebColors.primary, borderRadius: 8,
    paddingHorizontal: 6, paddingVertical: 2,
  },
  defaultTagText: { fontSize: 12, fontWeight: '800', color: '#fff' },
  delBtnSm: {
    width: 28, height: 28, borderRadius: 8,
    backgroundColor: WebColors.dangerLight,
  ...
  label: { fontSize: 12, fontWeight: '700', color: WebColors.textSecondary, textTransform: 'uppercase', marginBottom: 4 },
  input: {
    borderWidth: 1.5, borderColor: WebColors.border, borderRadius: 8,
    paddingHorizontal: 10, height: 36, fontSize: 12,
    color: WebColors.text, backgroundColor: WebColors.gray50,
  },
  ...
  unitDrop: {
    ...
    backgroundColor: '#fff',
    borderRadius: 12, borderWidth: 1.5, borderColor: WebColors.primary,
    ...Platform.select({ web: { boxShadow: '0 4px 20px rgba(0,0,0,0.15)' } as any }),
  },
  unitSearchRow: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
  ...
  unitSearchInput: {
    flex: 1, height: 32, fontSize: 12, color: WebColors.text,
  ...
  unitAddBtnText: { fontSize: 13, fontWeight: '700', color: '#fff' },
  unitItem: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingHorizontal: 12, paddingVertical: 9,
    borderBottomWidth: 1, borderBottomColor: WebColors.gray100,
  },
  unitItemActive: { backgroundColor: WebColors.primaryLight },
  unitItemText: { fontSize: 12, color: WebColors.text },
  unitItemTextActive: { color: WebColors.primary, fontWeight: '700' },
  colDefault: { alignItems: 'center', justifyContent: 'flex-start', minWidth: 44 },
  marginBox: {
    height: 36, borderRadius: 8, backgroundColor: WebColors.gray50,
  ...
  barcodeSection: {
    ...
    padding: 12, backgroundColor: '#FAFBFF', gap: 8,
  },
  barcodeHeader: {
  ...
  bcChip: {
    ...
    backgroundColor: WebColors.primaryLight, borderRadius: 20,
    paddingHorizontal: 10, paddingVertical: 5,
    borderWidth: 1, borderColor: WebColors.primary + '40',
  },
  bcText: { fontSize: 13, color: WebColors.primary, fontWeight: '700', letterSpacing: 0.5 },
  bcInputRow: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
    flex: 1, minWidth: 180,
  },
  bcInput: {
    flex: 1, height: 32, borderWidth: 1.5, borderColor: WebColors.border, borderRadius: 8,
    paddingHorizontal: 10, fontSize: 12, color: WebColors.text,
    backgroundColor: '#fff',
  },
  bcAddBtn: {
    ...
    backgroundColor: WebColors.primaryLight,
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 1.5, borderColor: WebColors.primary,
  },
  bcEmpty: { fontSize: 13, color: WebColors.textDisabled, marginTop: 2 },
  // Legacy (kept for compat)
  row: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, backgroundColor: WebColors.gray50, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: WebColors.border, marginBottom: 8, alignItems: 'flex-start' },
  colDel: { alignItems: 'center', justifyContent: 'flex-start', minWidth: 32 },
  delBtn: { width: 32, height: 36, borderRadius: 8, backgroundColor: WebColors.dangerLight, alignItems: 'center', justifyContent: 'center' },
              ...
              <View style={fm.imageRow}>
                ...
                <View style={fm.imagePreview}>
                  {imageUrl ? (
                    ...
                    <Ionicons name="image-outline" size={40} color={WebColors.border} />
                  )}
                </View>
                <View style={{ flex: 1, gap: 8 }}>
                  ...
                  {Platform.OS === 'web' && (
                    <>
                      ...
                      >
                        <Ionicons name="folder-open-outline" size={14} color={WebColors.primary} />
                  ...
                  {imageUrl ? (
                    <TouchableOpacity onPress={() => setImageUrl('')}>
                      <Text style={{ fontSize: 13, color: WebColors.danger }}>✕ ลบรูป</Text>
                    </TouchableOpacity>
                  ) : null}
                </View>
              </View>
            ...
            <View style={fm.section}>
              ...
              <View style={[fm.grid2, { zIndex: 9999 }]}>
                <View style={fm.field}>
                  ...
                  <View style={{ position: 'relative', zIndex: 9999 }}>
                    ...
                    {showCatDropdown && (
                      <View style={{ position: 'absolute', top: 40, left: 0, right: 0, backgroundColor: '#fff', borderRadius: 8, borderWidth: 1, borderColor: '#E5E7EB', zIndex: 9999, maxHeight: 180, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 }}>
                        <ScrollView style={{ maxHeight: 180 }}>
                          {MOCK_CATEGORIES.filter(c => c.status === 'active').map(c => (
                            <TouchableOpacity key={c.id} style={{ paddingVertical: 9, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', backgroundColor: catId === c.id ? WebColors.primaryLight : '#fff' }} onPress={() => { setCatId(c.id); setShowCatDropdown(false); }}>
                              <Text style={{ fontSize: 12, color: catId === c.id ? WebColors.primary : Colors.text }}>{c.name}</Text>
                            </TouchableOpacity>
                          ))}
                        </ScrollView>
                      </View>
                ...
                <View style={fm.field}>
                  ...
                  <View style={{ position: 'relative', zIndex: 9999 }}>
                    ...
                    {showBrandDropdown && (
                      <View style={{ position: 'absolute', top: 40, left: 0, right: 0, backgroundColor: '#fff', borderRadius: 8, borderWidth: 1, borderColor: '#E5E7EB', zIndex: 9999, maxHeight: 180, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 }}>
                        <ScrollView style={{ maxHeight: 180 }}>
                          <TouchableOpacity style={{ paddingVertical: 9, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', backgroundColor: !brandId ? WebColors.primaryLight : '#fff' }} onPress={() => { setBrandId(''); setShowBrandDropdown(false); }}>
                            <Text style={{ fontSize: 12, color: !brandId ? WebColors.primary : Colors.text }}>ไม่ระบุ</Text>
                          </TouchableOpacity>
                          {MOCK_BRANDS.filter(b => b.status === 'active').map(b => (
                            <TouchableOpacity key={b.id} style={{ paddingVertical: 9, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', backgroundColor: brandId === b.id ? WebColors.primaryLight : '#fff' }} onPress={() => { setBrandId(b.id); setShowBrandDropdown(false); }}>
                              <Text style={{ fontSize: 12, color: brandId === b.id ? WebColors.primary : Colors.text }}>{b.name}</Text>
                            </TouchableOpacity>
                          ))}
                        </ScrollView>
                      </View>
              ...
              <View style={fm.switchRow}>
                ...
                <View style={{ flexDirection: 'row', gap: 8 }}>
                  <TouchableOpacity
                    style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: productType !== 'service' ? WebColors.primary : WebColors.gray100, borderWidth: 1, borderColor: productType !== 'service' ? WebColors.primary : WebColors.border }}
                    onPress={() => setProductType('general')}
                  >
                    <Text style={{ fontSize: 13, fontWeight: '600', color: productType !== 'service' ? '#fff' : WebColors.textSecondary }}>สินค้า</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={{ paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, backgroundColor: productType === 'service' ? '#7C3AED' : WebColors.gray100, borderWidth: 1, borderColor: productType === 'service' ? '#7C3AED' : WebColors.border }}
                    onPress={() => setProductType('service')}
                  >
                    <Text style={{ fontSize: 13, fontWeight: '600', color: productType === 'service' ? '#fff' : WebColors.textSecondary }}>บริการ</Text>
                  </TouchableOpacity>
                </View>
              </View>
            ...
            <View style={fm.section}>
              <View style={fm.sectionHeader}>
                ...
                <TouchableOpacity style={fm.addUOMBtn} onPress={addUOM}>
                  <Ionicons name="add" size={14} color={WebColors.primary} />
...
const fm = StyleSheet.create({
  ...
  sheet: { backgroundColor: '#fff', borderRadius: 14, padding: 24, width: 720, maxHeight: '92%' as any, gap: 0, overflow: 'visible' as any },
  header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  title: { fontSize: 14, fontWeight: '700', color: WebColors.text },
  // Sections
  section: { marginBottom: 20, gap: 10, overflow: 'visible' as any, zIndex: 1 },
  sectionHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  sectionTitle: { fontSize: 12, fontWeight: '700', color: WebColors.text },
  // Image
  imageRow: { flexDirection: 'row', gap: 16, alignItems: 'flex-start' },
  imagePreview: {
    width: 120, height: 120, borderRadius: 12,
    borderWidth: 2, borderColor: WebColors.border, borderStyle: 'dashed',
    backgroundColor: WebColors.gray50,
  ...
  fileBtnText: { fontSize: 12, fontWeight: '600', color: WebColors.primary },
  // Fields
  ...
  autoBtn: {
    ...
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 1, borderColor: WebColors.primary,
  },
  autoBtnText: { fontSize: 13, fontWeight: '700', color: WebColors.primary },
  catChip: {
    paddingHorizontal: 10, paddingVertical: 6, borderRadius: 14,
    backgroundColor: '#fff', borderWidth: 1, borderColor: WebColors.border,
  },
  catChipActive: { backgroundColor: WebColors.primary, borderColor: WebColors.primary },
  catChipText: { fontSize: 13, color: WebColors.textSecondary, fontWeight: '600' },
  switchRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 6, borderTopWidth: 1, borderTopColor: WebColors.border,
  },
  // UOM
  addUOMBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 4,
  ...
  addUOMText: { fontSize: 13, fontWeight: '700', color: WebColors.primary },
  uomHintRow: { flexDirection: 'row', gap: 8, paddingHorizontal: 12, paddingBottom: 4 },
  uomHint: { flex: 1, fontSize: 12, fontWeight: '700', color: WebColors.textDisabled, textTransform: 'uppercase' },
  // Actions
  actions: {
  ...
  saveBtn: { flex: 2, height: 44, borderRadius: 12, backgroundColor: WebColors.primary, alignItems: 'center', justifyContent: 'center' },
    ...
    <View style={ms.root}>
      ...
      <View style={ms.tabBar}>
        {([
          ...
          >
            <Ionicons name={t.icon as any} size={14} color={masterTab === t.key ? WebColors.primary : WebColors.textSecondary} />
      ...
      {masterTab === 'unit' && (
        <View style={ms.panel}>
          ...
          <View style={ms.table}>
              ...
              return (
                <View key={u.id} style={[ms.tr, idx % 2 === 1 && ms.trAlt]}>
                  ...
                  <View style={{ flex: 2, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    ...
                    {u.isBuiltIn && (
                      <View style={{ backgroundColor: WebColors.gray100, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 2 }}>
                        <Text style={{ fontSize: 12, color: WebColors.textSecondary, fontWeight: '700' }}>มาตรฐาน</Text>
                      </View>
                    )}
                  </View>
                  {/* ใช้ในสินค้า */}
                  <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                    {inUse ? (
                      <>
                        <Ionicons name="cube-outline" size={13} color={WebColors.primary} />
                        <Text style={[ms.td, { color: WebColors.primary }]}>ใช้งานอยู่</Text>
                      </>
                    ) : (
                      <Text style={[ms.td, { color: WebColors.textDisabled }]}>—</Text>
                    )}
                  </View>
                  {/* สถานะ */}
                  <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                    ...
                    />
                    <Text style={[ms.td, {
                      color: locked ? WebColors.textDisabled
                        : u.status === 'active' ? WebColors.success : WebColors.textSecondary,
                    }]}>
                  ...
                  <View style={[ms.tdActions, { flex: 1.2 }]}>
                    {editingUnit === u.id ? (
                      ...
                      >
                        <Ionicons name="pencil-outline" size={14} color={WebColors.primary} />
                      </TouchableOpacity>
                    )}
                    <TouchableOpacity
            ...
            {units.length === 0 && (
              <View style={ms.empty}>
                <Ionicons name="resize-outline" size={40} color={WebColors.border} />
                <Text style={ms.emptyText}>ยังไม่มีหน่วย</Text>
              </View>
            )}
          </View>
          <Text style={ms.legend}>
            🔒 หน่วยที่มีสินค้าใช้งานอยู่ — แก้ไข/ปิดการใช้งาน/ลบไม่ได้  ·  หน่วยมาตรฐาน (badge) ลบไม่ได้
          </Text>
        </View>
      )}
      ...
      {masterTab === 'brand' && (
        <View style={ms.panel}>
              ...
              <View style={ms.inlineFormRow}>
                ...
                <View style={ms.inlineStatusRow}>
                  ...
                  <Switch
                    ...
                    trackColor={{ true: WebColors.success, false: WebColors.border }}
                  />
                  <Text style={[ms.inlineStatusText, { color: newBrandStatus === 'active' ? WebColors.success : WebColors.textSecondary }]}>
          ...
          <View style={ms.table}>
              ...
              <View key={b.id} style={[ms.tr, idx % 2 === 1 && ms.trAlt]}>
                ...
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Switch
                    ...
                    trackColor={{ true: WebColors.success, false: WebColors.border }}
                  />
                  <Text style={[ms.td, { color: b.status === 'active' ? WebColors.success : WebColors.textSecondary }]}>
            ...
            {brands.length === 0 && (
              <View style={ms.empty}>
                <Ionicons name="ribbon-outline" size={40} color={WebColors.border} />
      ...
      {masterTab === 'category' && (
        <View style={ms.panel}>
              ...
              <View style={ms.inlineFormRow}>
                ...
                <View style={ms.inlineStatusRow}>
                  ...
                  <Switch
                    ...
                    trackColor={{ true: WebColors.success, false: WebColors.border }}
                  />
                  <Text style={[ms.inlineStatusText, { color: newCategoryStatus === 'active' ? WebColors.success : WebColors.textSecondary }]}>
          ...
          <View style={ms.table}>
              ...
              <View key={c.id} style={[ms.tr, idx % 2 === 1 && ms.trAlt]}>
                ...
                <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  <Switch
                    ...
                    trackColor={{ true: WebColors.success, false: WebColors.border }}
                  />
                  <Text style={[ms.td, { color: c.status === 'active' ? WebColors.success : WebColors.textSecondary }]}>
            ...
            {categories.length === 0 && (
              <View style={ms.empty}>
                <Ionicons name="grid-outline" size={40} color={WebColors.border} />
...
const ms: any = StyleSheet.create({
  root: { flex: 1 },
  // Sub-tabs
  tabBar: {
  ...
  tabActive: { borderBottomColor: WebColors.primary },
  ...
  panelTitle: { fontSize: 13, fontWeight: '700', color: WebColors.text },
  panelSub: { fontSize: 13, color: WebColors.textSecondary, marginTop: 2 },
  // Buttons
  addBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 5,
    backgroundColor: WebColors.primary, borderRadius: 8,
  ...
  cancelBtnSmText: { fontSize: 12, color: WebColors.textSecondary },
  // Add unit row
  addUnitRow: {
  ...
  addUnitInput: {
    flex: 1, height: 36, borderWidth: 1.5, borderColor: WebColors.border,
  ...
  addUnitConfirmText: { fontSize: 12, fontWeight: '700', color: '#fff' },
  addUnitCancel: {
    borderWidth: 1.5, borderColor: WebColors.border, borderRadius: 8,
    paddingHorizontal: 12, paddingVertical: 8,
  },
  addUnitCancelText: { fontSize: 12, color: WebColors.textSecondary },
  // Chips
  ...
  chipUsed: { borderColor: WebColors.primary, backgroundColor: WebColors.primaryLight },
  chipLabel: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  chipText: { fontSize: 12, color: WebColors.text, fontWeight: '500' },
  chipUsedDot: {
  ...
  chipEditInput: {
    width: 90, height: 28, borderWidth: 1.5, borderColor: WebColors.primary,
    borderRadius: 8, paddingHorizontal: 6, fontSize: 13, color: WebColors.text,
  },
  tooltip: {
    position: 'absolute' as any,
    bottom: '120%' as any,
  ...
  tooltipText: { fontSize: 13, color: '#fff' },
  legend: { fontSize: 13, color: WebColors.textSecondary, marginTop: 4 },
  // Inline form
  inlineForm: {
    backgroundColor: WebColors.gray50,
    borderRadius: 12, borderWidth: 1, borderColor: WebColors.border,
    padding: 14, gap: 10,
  },
  inlineFormTitle: { fontSize: 12, fontWeight: '700', color: WebColors.text },
  ...
  inlineLabel: { fontSize: 13, color: WebColors.textSecondary, fontWeight: '600' },
  inlineStatusText: { fontSize: 13, fontWeight: '600' },
  // Table
  table: {
    backgroundColor: '#fff', borderRadius: 12,
    borderWidth: 1, borderColor: WebColors.border, overflow: 'hidden' as any,
  },
  thead: {
  ...
  th: { fontSize: 13, fontWeight: '700', color: WebColors.textSecondary, textTransform: 'uppercase' as any },
  tr: {
    flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 12,
    borderBottomWidth: 1, borderBottomColor: WebColors.border, alignItems: 'center',
  },
  trAlt: { backgroundColor: WebColors.gray50 },
  td: { fontSize: 12, color: WebColors.textSecondary },
  tdText: { fontSize: 12, fontWeight: '500', color: WebColors.text },
  trInput: {
    height: 34, borderWidth: 1.5, borderColor: WebColors.primary,
    borderRadius: 8, paddingHorizontal: 8, fontSize: 12,
    color: WebColors.text, backgroundColor: WebColors.primaryLight,
  },
  ...
  actionBtnSuccess: {
    width: 30, height: 30, borderRadius: 8,
    backgroundColor: WebColors.successLight,
    alignItems: 'center', justifyContent: 'center',
  },
  actionBtnDanger: {
    width: 30, height: 30, borderRadius: 8,
...
// ─── Main Screen ──────────────────────────────────────────────────────────────
export const WebProductScreen: React.FC = () => {
  ...
  const getStatusBadge = (p: ProductMaster) => {
    if (p.status === 'inactive') return { label: 'ปิดใช้',    color: WebColors.textSecondary, bg: WebColors.gray100 };
    if (p.stockQty === 0)        return { label: 'หมดสต๊อก',  color: WebColors.danger,        bg: WebColors.dangerLight };
    if (p.stockQty <= p.minStock) return { label: 'ใกล้หมด',  color: WebColors.warning,       bg: WebColors.warningLight };
    return                              { label: 'ใช้งาน',    color: WebColors.success,       bg: WebColors.successLight };
  };
    ...
    <View style={s.root}>
      ...
      <View style={s.subNav}>
        {[
          ...
          >
            <Ionicons name={t.icon as any} size={15} color={subView === t.key ? WebColors.primary : WebColors.textSecondary} />
        ...
        <>
          ...
          <View style={s.toolbar}>
            <View style={s.searchBar}>
              ...
              {search !== '' && (
                <TouchableOpacity onPress={() => setSearch('')}>
                  <Ionicons name="close-circle" size={15} color={WebColors.textSecondary} />
                </TouchableOpacity>
              )}
            </View>
            ...
            <TouchableOpacity style={s.exportBtn}>
              <Ionicons name="cloud-download-outline" size={14} color={WebColors.textSecondary} />
          ...
          {isMobile ? (
            <View style={{ gap: 10 }}>
              {filtered.map((p) => {
                ...
                return (
                  <TouchableOpacity key={p.id} style={{ backgroundColor: '#fff', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#F0E2DA' }} onPress={() => { setEditProduct(p); setShowModal(true); }}>
                    <View style={{ flexDirection: 'row', gap: 10, alignItems: 'center' }}>
                      {p.image ? (
                        ...
                        <View style={{ width: 40, height: 40, borderRadius: 8, backgroundColor: '#F5F5F5', alignItems: 'center', justifyContent: 'center' }}>
                          <Ionicons name="cube-outline" size={18} color={WebColors.border} />
                        </View>
                      )}
                      <View style={{ flex: 1 }}>
                        <Text style={{ fontSize: 15, fontWeight: '600', color: '#3A2E2B' }} numberOfLines={1}>{p.name}</Text>
                        <Text style={{ fontSize: 12, color: '#6B5B57' }}>{p.code} · {p.categoryName}</Text>
                      </View>
                    ...
                    <View style={{ flexDirection: 'row', gap: 12, marginTop: 8, alignItems: 'center' }}>
                      <Text style={{ fontSize: 14, fontWeight: '700', color: WebColors.primary }}>฿{p.salePrice}</Text>
                      <Text style={{ fontSize: 12, color: p.stockQty <= p.minStock ? WebColors.danger : '#6B5B57', fontWeight: p.stockQty <= p.minStock ? '700' : '400' }}>คงเหลือ {p.stockQty} {p.unit}</Text>
                    </View>
                  </TouchableOpacity>
                );
              })}
              {filtered.length === 0 && (
                <View style={s.empty}>
                  <Ionicons name="cube-outline" size={48} color={WebColors.border} />
                  ...
                  <View style={[s.tr, index % 2 === 1 && s.trAlt]}>
                    ...
                    <View style={[s.tdThumb, { flex: 0.5 }]}>
                      {p.image ? (
                        ...
                        <View style={s.thumbPlaceholder}>
                          <Ionicons name="cube-outline" size={16} color={WebColors.border} />
                        </View>
                      )}
                    </View>
                    {/* รหัส / บาร์โค้ด */}
                    ...
                    <View style={{ flex: 0.7, alignItems: 'flex-start', justifyContent: 'center' }}>
                      <View style={s.uomBadge}>
                        <Ionicons name="layers-outline" size={10} color={WebColors.primary} />
                        <Text style={s.uomBadgeText}>{p.uoms.length} หน่วย</Text>
                      </View>
                      <Text style={s.tdSub}>{p.unit}</Text>
                    </View>
                    {/* ราคา */}
                    <Text style={[s.td, { flex: 0.9, color: WebColors.primary, fontWeight: '700' }]}>
                      ฿{p.salePrice}
                    </Text>
                    {/* คงเหลือ */}
                    <View style={{ flex: 0.8, justifyContent: 'center' }}>
                      <Text style={[
                        s.td,
                        p.stockQty <= p.minStock && { color: WebColors.danger, fontWeight: '700' },
                      ]}>
                        {p.stockQty} {p.unit}
                      </Text>
                    </View>
                    ...
                    <View style={[s.tdActions, { flex: 0.9 }]}>
                      ...
                      >
                        <Ionicons name="pencil-outline" size={13} color={WebColors.primary} />
                      </TouchableOpacity>
                      <TouchableOpacity
                        style={[s.actionBtn, { backgroundColor: WebColors.dangerLight }]}
        ...
        <View style={s.subContent}>
          ...
          <View style={{ backgroundColor: '#fff', borderRadius: 12, borderWidth: 1, borderColor: Colors.border, overflow: 'hidden' }}>
            ...
            {products.slice(0, 15).map((p, i) => {
              ...
              return (
                <View key={p.id} style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 10, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: Colors.border, backgroundColor: i % 2 === 1 ? Colors.background : '#FFF' }}>
                  ...
                  <Text style={{ flex: 1, fontSize: 13, color: WebColors.primary, textAlign: 'right' }}>฿{fmt(p.salePrice * 0.95)}</Text>
        ...
        <View style={s.subContent}>
          ...
          <View style={s.importGrid}>
            <View style={s.importCard}>
              <Ionicons name="cloud-upload-outline" size={32} color={WebColors.primary} />
            ...
            <View style={s.importCard}>
              <Ionicons name="cloud-download-outline" size={32} color={WebColors.success} />
              <Text style={s.importCardTitle}>Export สินค้าทั้งหมด</Text>
              <Text style={s.importCardSub}>{products.length} รายการ</Text>
              <TouchableOpacity style={[s.importBtn, { backgroundColor: WebColors.success }]}>
                <Text style={s.importBtnText}>Export Excel</Text>
              </TouchableOpacity>
            </View>
            <View style={s.importCard}>
              <Ionicons name="document-text-outline" size={32} color={WebColors.warning} />
              <Text style={s.importCardTitle}>Download Template</Text>
              <Text style={s.importCardSub}>แบบฟอร์มสำหรับกรอกข้อมูล</Text>
              <TouchableOpacity style={[s.importBtn, { backgroundColor: WebColors.warning }]}>
...
const s = StyleSheet.create({
  ...
  subNav: {
    flexDirection: 'row', backgroundColor: '#fff',
    borderBottomWidth: 1, borderBottomColor: WebColors.border, marginBottom: 16,
  },
  subTab: {
    flexDirection: 'row', alignItems: 'center', gap: 6,
  ...
  subTabActive: { borderBottomColor: WebColors.primary },
  subTabText: { fontSize: 12, color: WebColors.textSecondary },
  subTabTextActive: { color: WebColors.primary, fontWeight: '700' },
  // Toolbar
  toolbar: {
  ...
  chipText: { fontSize: 13, color: WebColors.textSecondary },
  chipTextActive: { color: '#fff', fontWeight: '700' },
  statusToggle: {
    flexDirection: 'row', backgroundColor: WebColors.gray100,
    borderRadius: 8, padding: 3, gap: 2,
  },
  statusBtn: { paddingHorizontal: 12, paddingVertical: 5, borderRadius: 8 },
  statusBtnActive: { backgroundColor: '#fff' },
  statusBtnText: { fontSize: 13, color: WebColors.textSecondary },
  statusBtnTextActive: { color: WebColors.text, fontWeight: '700' },
  exportBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 5,
    borderWidth: 1, borderColor: WebColors.border,
  ...
  exportBtnText: { fontSize: 12, color: WebColors.textSecondary },
  addBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 5,
    backgroundColor: WebColors.primary, borderRadius: 8, paddingHorizontal: 16, height: 40,
  },
  addBtnText: { fontSize: 12, fontWeight: '700', color: '#fff' },
  // Table
  table: {
    backgroundColor: '#fff', borderRadius: 12,
    borderWidth: 1, borderColor: WebColors.border, flex: 1,
  },
  thead: {
    flexDirection: 'row', backgroundColor: WebColors.gray50,
    paddingHorizontal: 16, paddingVertical: 10,
  ...
  uomBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 3,
    backgroundColor: WebColors.primaryLight, borderRadius: 12,
    paddingHorizontal: 6, paddingVertical: 2, alignSelf: 'flex-start',
  },
  uomBadgeText: { fontSize: 12, fontWeight: '700', color: WebColors.primary },
  ...
  subTitle: { fontSize: 14, fontWeight: '700', color: WebColors.text },
  ...
  importCardTitle: { fontSize: 12, fontWeight: '700', color: WebColors.text },
  importCardSub: { fontSize: 12, color: WebColors.textSecondary, textAlign: 'center' },
  importBtn: {
    backgroundColor: WebColors.primary, borderRadius: 8,
    paddingHorizontal: 20, paddingVertical: 10, marginTop: 4,
  },
  importBtnText: { fontSize: 12, fontWeight: '700', color: '#fff' },
  catGrid: { gap: 8 },
  catCard: {
  ...
  catName: { fontSize: 12, fontWeight: '600', color: WebColors.text },
  catCount: { fontSize: 13, color: WebColors.textSecondary },
});
