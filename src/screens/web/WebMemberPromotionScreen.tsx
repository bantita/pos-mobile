/**
 * WebMemberPromotionScreen — Member Promotion Engine
 * จัดการโปรโมชั่นเฉพาะสมาชิก (แยกจาก WebPromotionScreen)
...
const getStatusInfo = (status: MemberPromoStatus) => {
  switch (status) {
    case 'active': return { label: 'Active', color: WebColors.success, bg: WebColors.successLight };
    case 'paused': return { label: 'หยุดชั่วคราว', color: WebColors.warning, bg: WebColors.warningLight };
    case 'expired': return { label: 'หมดอายุ', color: WebColors.grayMedium, bg: WebColors.gray100 };
    case 'draft': return { label: 'แบบร่าง', color: WebColors.info, bg: WebColors.infoLight };
  }
};
...
const KPICard: React.FC<{ label: string; value: string | number; color?: string; icon: string }> = ({ label, value, color, icon }) => (
  <View style={styles.kpiCard}>
    <View style={[styles.kpiIconWrap, { backgroundColor: (color || WebColors.primary) + '15' }]}>
      <Ionicons name={icon as any} size={20} color={color || WebColors.primary} />
    </View>
    ...
    <View style={styles.formPanel}>
      <View style={styles.formHeader}>
        ...
        <TouchableOpacity onPress={onClose} style={styles.formCloseBtn}>
          <Ionicons name="close" size={22} color={WebColors.grayMedium} />
        </TouchableOpacity>
      </View>
      ...
      <ScrollView style={styles.formBody} showsVerticalScrollIndicator={false}>
          ...
          {ALL_PROMO_TYPES.map(t => {
            ...
            return (
              <TouchableOpacity
                ...
                disabled={!isPhase1}
              >
                <Ionicons name={t.icon as any} size={16} color={isSelected ? '#fff' : isPhase1 ? WebColors.primary : WebColors.textDisabled} />
                <Text style={[
                  styles.typeOptionText,
                  isSelected && styles.typeOptionTextActive,
                  !isPhase1 && { color: WebColors.textDisabled },
        ...
        {selectedType === 'member_price' && (
          <View>
            ...
            <TextInput style={styles.formInput} placeholder="รหัสสินค้า (คั่นด้วย ,)" placeholderTextColor={WebColors.textDisabled} />
            <Text style={styles.formLabel}>ส่วนลด (บาท)</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 3" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
          </View>
        )}
        {selectedType === 'level_discount' && (
          <View>
            <Text style={styles.formLabel}>ระดับ & เปอร์เซ็นต์ส่วนลด</Text>
            <TextInput style={styles.formInput} placeholder="Silver: 3%, Gold: 5%, Platinum: 10%, VIP: 15%" placeholderTextColor={WebColors.textDisabled} />
          </View>
        )}
        {selectedType === 'birthday' && (
          <View>
            <Text style={styles.formLabel}>ส่วนลด (%)</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 20" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
            <Text style={styles.formLabel}>ส่วนลดสูงสุด (บาท)</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 500" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
          </View>
        )}
        {selectedType === 'welcome' && (
          <View>
            <Text style={styles.formLabel}>แต้มที่ได้รับ</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 100" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
            <Text style={styles.formLabel}>คูปอง (บาท)</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 50" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
          </View>
        )}
        {selectedType === 'bonus_points' && (
          <View>
            ...
            <TextInput style={styles.formInput} placeholder="เช่น coffee, bakery" placeholderTextColor={WebColors.textDisabled} />
          </View>
        )}
        {selectedType === 'stamp' && (
          <View>
            <Text style={styles.formLabel}>เป้าหมายแสตมป์ (ดวง)</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 10" placeholderTextColor={WebColors.textDisabled} keyboardType="numeric" />
        ...
        {(selectedType === 'points_to_discount' || selectedType === 'points_to_product') && (
          <View>
            ...
            <TextInput style={styles.formInput} placeholder="เช่น 100, 500, 1000" placeholderTextColor={WebColors.textDisabled} />
            <Text style={styles.formLabel}>มูลค่าที่ได้</Text>
            <TextInput style={styles.formInput} placeholder="เช่น 10, 60, 150" placeholderTextColor={WebColors.textDisabled} />
          </View>
        )}
...
export const WebMemberPromotionScreen: React.FC = () => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.sidebar}>
        ...
        >
          <Ionicons name="apps-outline" size={16} color={activeCategory === 'all' ? WebColors.primary : WebColors.grayMedium} />
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            ...
            >
              <Ionicons name={cat.icon as any} size={16} color={isActive ? WebColors.primary : WebColors.grayMedium} />
      ...
      <View style={styles.content}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.contentInner}>
          ...
          <View style={styles.kpiRow}>
            <KPICard label="โปรทั้งหมด" value={totalPromos} color={WebColors.primary} icon="layers-outline" />
            <KPICard label="Active" value={activePromos} color={WebColors.success} icon="checkmark-circle-outline" />
            <KPICard label="ใช้แล้ว (ครั้ง)" value={totalUsage} color={WebColors.info} icon="people-outline" />
            <KPICard label="ส่วนลดรวม (฿)" value={totalReward} color={WebColors.purple} icon="cash-outline" />
          </View>
          ...
          <View style={styles.promoGrid}>
              ...
              <View key={promo.id} style={styles.promoCard}>
                ...
                <View style={styles.promoCardHeader}>
                  <View style={styles.promoTypeWrap}>
                    <View style={[styles.promoIconBg, { backgroundColor: WebColors.primaryLight }]}>
                      <Ionicons name={getTypeIcon(promo.type) as any} size={16} color={WebColors.primary} />
                    </View>
                ...
                <View style={styles.rewardRow}>
                  <Ionicons name="gift-outline" size={14} color={WebColors.success} />
                  <Text style={styles.rewardText}>{formatRewards(promo)}</Text>
                </View>

                {/* Date */}
                <View style={styles.dateRow}>
                  <Ionicons name="calendar-outline" size={13} color={WebColors.grayMedium} />
                ...
                <View style={styles.promoCardFooter}>
                  ...
                  >
                    <Ionicons
                      ...
                      size={14}
                      color={promo.status === 'active' ? WebColors.warning : WebColors.success}
                    />
                    <Text style={[
            ...
            {filteredPromos.length === 0 && (
              <View style={styles.emptyState}>
                <Ionicons name="search-outline" size={48} color={WebColors.textDisabled} />
...
const styles = StyleSheet.create({
  ...
  promoCardFooter: {
    ...
    alignItems: 'center',
    borderTopWidth: 1,
    borderTopColor: WebColors.cardBorder,
    paddingTop: 10,
  },
  statRow: {
    flexDirection: 'row',
  ...
  toggleBtnActive: {
    backgroundColor: WebColors.warningLight,
  },
  toggleBtnPaused: {
    backgroundColor: WebColors.successLight,
  },
