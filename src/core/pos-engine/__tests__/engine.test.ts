 ...
 * Run: npx tsx src/core/pos-engine/__tests__/engine.test.ts
 */
import { calculate } from '@/core/pos-engine/index';
import {
...
} from '@/core/pos-engine/types/index';
...
// ─── Test Data Helpers ────────────────────────────────────────────────────────
function makeCart(items: { code: string; qty: number; price: number; group?: string }[]): CartItem[] {
...
// ─── Test 13: Member segment condition ───────────────────────────────────────
console.log('\n📋 Test: Member segment condition');
(() => {
  const cart = makeCart([{ code: 'P001', qty: 1, price: 500 }]);
  const promo = makePromo({
    promotionId: 'PROMO-SEGMENT',
    name: 'โปรเฉพาะ Segment VIP',
    conditions: [{ type: 'member', params: { segments: ['vip'] } }],
    benefits: [{ type: 'bill_discount', params: { amount: 50 } }],
  });
  const rejected = calculate({
    ...BASE_REQUEST as CalculationRequest,
    storeCode: 'STORE-001',
    businessDateTime: '2026-06-17T14:30:00+07:00',
    cartItems: cart,
    subtotal: 500,
    memberSegments: ['regular'],
    promotionCandidates: [promo],
  });
  assert(rejected.appliedBenefits.length === 0, 'Wrong segment → no benefit');
  ...
  assert(accepted.appliedBenefits.length === 1, 'Matching segment → promo applied');
  assertApprox(accepted.finalTotals.finalPayable, 450, 0.01, 'Segment discount applied');
})();
